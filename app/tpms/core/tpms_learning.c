...
volatile uint8_t g_tpms_learning_last_result =TPMS_LEARNING_RESULT_IDLE;

volatile uint8_t g_tpms_learning_last_success_result =TPMS_LEARNING_RESULT_IDLE;

volatile uint8_t g_tpms_learning_last_position = 0;

volatile uint8_t g_tpms_learning_last_id[4] ={0,0,0,0};

volatile uint8_t g_tpms_learning_last_function_reuse[2] ={0,0};
...
volatile uint8_t g_tpms_learning_wheel_learn_flag[4] ={0,0,0,0};
...
volatile uint8_t g_tpms_learning_white_source_index[4] ={0xFF,0xFF,0xFF,0xFF};
...
static const uint8_t s_learning_position_table[4]={TPMS_APP_POS_LF,TPMS_APP_POS_RF,TPMS_APP_POS_RR,TPMS_APP_POS_LR};
...
void TpmsLearning_init(void){uint8_t i;
...
void TpmsLearning_startManual(void){TpmsLearning_init();
...
void TpmsLearning_stopManual(void){g_tpms_learning_mode_active=0;}



bool TpmsLearning_isActive(void){return (g_tpms_learning_mode_active != 0);}
...
static bool TpmsLearning_checkFunctionReuse(const uint8_t *mfr,uint8_t len){

if(mfr == NULL)
    return false;


if(len < TPMS_LEARNING_MFR_DATA_LEN)
    return false;
...
if((mfr[TPMS_LEARNING_FUNCTION_REUSE1_INDEX] &
    TPMS_LEARNING_TRIGGER_BIT_MASK) != 0U)
{
    return true;
}
...
static bool TpmsLearning_isDuplicate(uint8_t source_index){uint8_t i;
...
static void TpmsLearning_makeId(const uint8_t *mfr_data,uint8_t *id){
...
id[0]=mfr_data[TPMS_LEARNING_SENSOR_ID_INDEX + 0U];
id[1]=mfr_data[TPMS_LEARNING_SENSOR_ID_INDEX + 1U];
id[2]=mfr_data[TPMS_LEARNING_SENSOR_ID_INDEX + 2U];
id[3]=mfr_data[TPMS_LEARNING_SENSOR_ID_INDEX + 3U];
...
static void TpmsLearning_setPositionFlag(uint8_t position){
...
static void TpmsLearning_notifyApp(uint8_t wheel_index,
...
TpmsBleService_queueFfb2SensorRaw(
    ...
    mfr_data[TPMS_LEARNING_PRESSURE_INDEX],

    mfr_data[TPMS_LEARNING_TEMPERATURE_INDEX],

    mfr_data[TPMS_LEARNING_INFO_INDEX],
...
static void TpmsLearning_sendClearRecordToApp(uint8_t wheel_index){uint8_t info;
...
void TpmsLearning_notifyClearLearnedRecords(void){uint8_t i;
...
void TpmsLearning_onTpmsAdv(uint8_t whitelist_index,const uint8_t *addr,uint8_t addr_type,int8_t rssi,const uint8_t *mfr_data,uint8_t mfr_len){
